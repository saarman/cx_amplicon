...
#R1 is forward reads FASTQ and R2 is reverse reads FASTQ
#fm.bwa_index indexes the reference genome
#faidx generates an index file allowing access to target portions without reading entire file

#more data input, args.ref reference file
...
# for loop where sample is i
# fastqc to identify issues in raw data beforehand
...
# submitting to a cluster, rather than doing work in python
# possibility of not getting accurate text match
# cluster - bwa to align to reference genome - bam to organize the alignment data to prepare for variant calling
...
# vcf variant called file w snps, 
# creating a list of vcf files
# Using another for loop for variant calling in each sample
# creates a new list file "sample_vcf_list.txt"
# merges into a combined vcf file to prepare for data pulling
...
#vcf will filter out based on dif stats for higher qaulity snps
#results in data for: chromosome, position on the chromosome, reference allele, alternative allele, quality score of the variant, genotype, translated genotype, read depth, allele depth, 
...
# creating a bed file, with chromosome, and position labeled
...
# implements everything and loops back to the beginning main function if needed
# Set up the parser
...
parser.add_argument('--trim',action="store_true",help='Perform triming')  #trimming
parser.add_argument('--trim-qv',default=20,type=int,help='Quality value to use in the sliding window analysis')  # quality threshold for trimming
parser.add_argument('--min-base-qual',default=30,type=int,help='Minimum base quality to use by freebayes')  # minimum base quality for variant calling
parser.add_argument('--min-adf',type=float,help='Set a minimum frequency for a mixed call')          # minimum allele frequency for making mixed calls
parser.add_argument('--sample-prefix',default="",help=argparse.SUPPRESS)                            #  adds prefixes to the samples
parser.add_argument('--vcf-files',type=str,nargs="+",help='VCF files with positions to include (optional)')    
parser.add_argument('--min-variant-qual',default=30,type=int,help='Quality value to use in the sliding window analysis')   # minimum variant calling score for filtering results
parser.add_argument('--min-sample-af',default=0.05,type=float,help='Quality value to use in the sliding window analysis')  # mimimum allele frequency for variant calls
parser.add_argument('--per-sample-only',action="store_true",help='Perform triming')                   # limits operations to analysis on a per sample basis 
...
parser.set_defaults(func=main)                                                                   # implements the above and loops back to the beginning main function
